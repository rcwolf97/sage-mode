...
const SAGE_MODE_VERSION = readSageModeVersion();
const SAGE_MODE_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/sage-mode-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'SAGE_MODE_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SAGE_MODE_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function readSageModeVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Packaged plugins may omit package.json; try host manifests next.
    }
  }
...
function brandMarkup() {
  const version = escapeHtmlText(SAGE_MODE_VERSION);
  const text = SAGE_MODE_TELEMETRY_DISABLED
    ? 'Prime Radiant Sage Mode v' + version
    : 'Sage Mode v' + version;
  const logo = SAGE_MODE_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SAGE_MODE_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(SAGE_MODE_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/rcwolf97/sage-mode">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
