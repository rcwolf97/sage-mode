...
**Two scores rest on unverified assumptions, and both should be read with their downside, not just their upside.** Cost control (10) assumes plugin-shipped subagents honor their `model` frontmatter — documented for `.cursor/agents/`, never stated for plugins. There is currently no code anywhere in `lib/` that checks which model actually served a request; `lib/consult/index.ts` only ever shells out to `claude`, and `lib/board/index.ts` displays whatever string is in a node's `model` field without verifying it against anything the host reports. **If the assumption is wrong, per-role model tiering isn't wired at all and cost control drops to about a 3** — prose intent with nothing behind it, the same failure mode the rubric's own "a 3 looks like" row describes. Parallel throughput (9) assumes `preToolUse`'s `tool_input` carries `file_path` for Write, which Cursor's docs never show. **If that assumption is wrong, lane enforcement isn't buildable and parallel throughput drops to about a 5.** Both are an hour of testing against a live Cursor session, and until that hour happens, treat both 10 and 9 as ceilings, not settled scores.
