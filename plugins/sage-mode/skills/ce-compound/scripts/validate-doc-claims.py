...
def unwrap_html_frontmatter(text: str) -> str:
    """If YAML lives in an HTML comment, return ---...--- plus the rest of the file.

    Accepts either a file that starts with --- (markdown) or a comment of the form
        <!--
        ---
        key: value
        ---
        -->
    anywhere near the top. The comment is stripped so callers can keep matching
    on a leading --- line.
    """
    m = re.search(r"<!--\s*\n---\n(.*?)\n---\s*\n-->", text, re.DOTALL)
    if not m:
...
def main(argv: list[str]) -> int:
    ...
    with open(doc_path) as f:
        text = unwrap_html_frontmatter(f.read())
    ...
    href_targets = MD_LINK_RE.findall(body) + HTML_HREF_RE.findall(body)
