...
export interface Finding {
  ...
  fingerprint?: string;
  specialist: string;
  /** Evidence ladder 1–5. Optional; unset keeps pre-ladder gate behavior. */
  rung?: number;
...
export function looksLikeCommandOutput(evidence: string): boolean {
  if (!evidence.trim()) return false;
  if (/```/.test(evidence)) return true;
  if (/^[\$%#>]\s/m.test(evidence)) return true;
  if (/\n\$\s/.test(evidence)) return true;
  return false;
}
...
export interface PriorRunEntry {
  disposition: "skipped" | "fixed";
  hash: string;
}

export type ReviewState = Record<string, PriorRunEntry>;

export function reviewStatePath(sprint: string, root?: string): string {
  return join(projectSageDir(root), "sprints", sprint, "review-state.json");
}

export function loadReviewState(sprint: string, root?: string): ReviewState {
  const p = reviewStatePath(sprint, root);
  if (!existsSync(p)) return {};
  try {
    return JSON.parse(readFileSync(p, "utf8")) as ReviewState;
  } catch {
    return {};
  }
}

export function saveReviewState(sprint: string, state: ReviewState, root?: string): void {
  const p = reviewStatePath(sprint, root);
...
export function applyCrossRunDedup(findings: Finding[], state: ReviewState, fileHash: (path: string) => string | undefined): Finding[] {
  return findings.filter((f) => {
    const fp = fingerprint(f);
    const prior = state[fp];
    if (!prior || prior.disposition !== "skipped") return true;
...
export function contentHash(path: string): string | undefined {
...
export function sessionReviewDir(slug: string, root?: string): string {
