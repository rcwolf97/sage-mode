...
export const ROLES = ["frontend", "backend", "data", "infra", "ai", "design"];
const ID = /^n[0-9]+$/;
...
export function validate(dag) {
    ...
    if (!SPRINT.test(dag.sprint || ""))
        v.push({ code: "schema", message: `sprint id invalid: ${dag.sprint}` });
    if (!dag.base)
        v.push({ code: "schema", message: "base is required" });
    if (!Array.isArray(dag.nodes) || dag.nodes.length < 1)
    ...
    for (const n of dag.nodes || []) {
        ...
        if (ids.has(n.id))
            v.push({ code: "schema", message: `duplicate id ${n.id}` });
        ids.add(n.id);
        if (!n.title || n.title.length < 8 || n.title.length > 120) {
            v.push({ code: "schema", message: `title length on ${n.id}` });
        }
        if (!ROLES.includes(n.role))
            v.push({ code: "schema", message: `bad role on ${n.id}` });
        if (!n.owns?.length)
            v.push({ code: "schema", message: `owns required on ${n.id}` });
        if (!n.acceptance?.length)
            v.push({ code: "schema", message: `acceptance required on ${n.id}` });
        if (!n.verify)
    ...
    for (const n of dag.nodes || []) {
        for (const d of n.depends_on || []) {
            if (!idSet.has(d))
                v.push({ code: "D1", message: `${n.id} depends on missing ${d}`, nodes: [n.id, d] });
        }
    }
...
function segmentCompatible(x, y) {
    ...
    if (x === "*" || y === "*")
        return true;
    if (!hasWildcard(x) && !hasWildcard(y))
        return x === y;
    if (!hasWildcard(x))
...
export function expandAgainstTree(glob, treePaths) {
...
function translateBracketOrQuestion(g, i) {
    const c = g[i];
    if (c === "?")
        return { chunk: "[^/]", next: i + 1 };
    if (c !== "[")
        return null;
    let j = i + 1;
    let cls = "[";
    ...
    if (j >= g.length)
        return null; // unterminated class — treat `[` as literal below
    cls += "]";
...
export function worktree(nodeId, dag, cwd) {
    const root = gitRoot(cwd) || cwd || process.cwd();
...
export function megafileFlag(dag, cwd) {
    const root = gitRoot(cwd) || cwd || process.cwd();
    const v = [];
    for (const n of dag.nodes) {
...
export function validateFile(path, cwd) {
