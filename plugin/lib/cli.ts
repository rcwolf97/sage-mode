...
import { run as evidenceRun, check as evidenceCheck, trust, wtree, type EvidenceRecord } from "./evidence/index.js";
...
async function main(argv: string[]): Promise<number> {
  ...
  if (cmd === "evidence") {
    ...
    if (sub === "run") {
      ...
      if (!label || !cmdv.length) fail("sage evidence run --label L [--node N] [--sprint S | --session] [--grade G] [--allow-unignored] -- <cmd>");
      const gradeRaw = opt(rest, "grade") as EvidenceRecord["grade"] | undefined;
    ...
    if (sub === "check") {
      const label = opt(rest, "label");
      if (!label) fail("sage evidence check --label L [--sprint S | --session]");
  ...
  if (cmd === "review") {
    ...
    if (sub === "doc") {
      const file = rest.find((a, i) => i > 0 && !a.startsWith("-"));
      if (!file) fail("sage review doc <file>");
      if (!existsSync(file)) fail(`sage review doc: ${file} does not exist`);
  ...
  if (cmd === "board") {
    ...
    if (sub === "render") {
      if (!sprint) fail("sage board render [--sprint S]");
  ...
  if (cmd === "ground") {
    ...
    if (!file) fail("sage ground <file>");
    if (!existsSync(file)) fail(`sage ground: ${file} does not exist`);
