...
function placeholder(kind, matchedLength) {
...
function isClaimed(claims, start, end) {
...
// -----------------------------------------------------------------------
function detectPrivateKey(text, claims) {
...
function detectJwt(text, claims) {
...
function wholeMatchDetector(re, kind) {
    return (text, claims) => {
        const out = [];
        let m;
        ...
        while ((m = r.exec(text))) {
            ...
            if (m[0].length === 0)
                r.lastIndex++; // guard against zero-width matches looping forever
        }
        return out;
    };
}
const detectAwsKey = wholeMatchDetector(/\b(?:AKIA|ASIA)[0-9A-Z]{16}\b/g, "aws-key");
const detectGithubToken = wholeMatchDetector(/\b(?:ghp|gho|ghs|ghr|ghu)_[A-Za-z0-9]{36,255}\b|\bgithub_pat_[A-Za-z0-9_]{20,255}\b/g, "github-token");
...
function valueGroupDetector(re, kind) {
    return (text, claims) => {
        const out = [];
        let m;
        const r = new RegExp(re.source, re.flags.includes("g") ? re.flags : re.flags + "g");
...
function detectAssignments(text, claims) {
...
function detectHighEntropy(text, claims) {
