...
function backtickTokens(raw: string): string[] {
  const out: string[] = [];
  const re = /`([^`]*)`/gs;
...
function checkReferenceIntegrity(root: string, skillMd: string, skillDir: string, raw: string): LintIssue[] {
  const issues: LintIssue[] = [];
  for (const token of backtickTokens(raw)) {
    let base: string | null = null;
    if (token.startsWith("references/")) base = skillDir;
    else if (SHARED_ROOT_PREFIXES.some((p) => token.startsWith(p))) base = root;
    else if (/^skills\/(?:[^/]+\/)+references\//.test(token)) base = root;
    else continue;
...
function checkSelfContainment(skillMd: string, skillDir: string, skillsRoot: string, raw: string): LintIssue[] {
  const issues: LintIssue[] = [];
  ...
  for (const token of backtickTokens(body)) {
    if (token.startsWith("/") || token.startsWith("~")) {
      ...
      const isReferenceShaped =
        /(^|\/)(references|templates|schemas|profiles)(\/|$)/.test(withoutHome) || withoutHome.startsWith("/skills/");
    ...
    if (token.startsWith("skills/")) {
      if (token.includes("*")) continue; // a glob over the whole catalog (e.g. `skills/**/SKILL.md`), not a reference to one skill
      const rest = token.slice("skills/".length);
      if (rest === ownRelDir || rest.startsWith(`${ownRelDir}/`)) continue; // self-reference, fine
      if (/^(?:[^/]+\/)+references\//.test(rest)) continue; // allowlisted sibling-dispatch form
      issues.push({
        file: skillMd,
        rule: "self-containment",
...
function normalizeConductText(s: string): string {
  return s
    .toLowerCase()
    .replace(/[`*_]/g, "")
    .replace(/[^a-z0-9\s]/g, " ")
...
// doesn't cover.
function conductSentences(body: string): { raw: string; norm: string }[] {
  ...
  for (const p of parts) {
    ...
    if (norm.split(" ").filter(Boolean).length < 5) continue; // too short to be a "distinctive" sentence
...
function conductBlocks(body: string): ConductBlock[] {
  const sentences = conductSentences(body);
  const out: ConductBlock[] = [];
  for (let i = 0; i < sentences.length - 1; i++) {
    const block = `${sentences[i].norm} ${sentences[i + 1].norm}`;
    if (block.split(" ").filter(Boolean).length < 8) continue;
...
function conductHeadings(body: string): string[] {
  return body
    .split(/\r?\n/)
    .filter((l) => /^##\s+/.test(l))
...
// references.
function extractSpecialistTableNames(skillRaw: string): string[] {
...
function checkRosterCoverage(root: string): LintIssue[] {
  ...
  let m: RegExpExecArray | null;
  while ((m = pathRe.exec(concatenated))) pathNames.add(m[1]);
...
export function findDocsRootLiterals(root = pluginRoot): LintIssue[] {
  const issues: LintIssue[] = [];
...
export function lint(root = pluginRoot): LintIssue[] {
  ...
  for (const skillMd of walk(skillsRoot, (n) => n === "SKILL.md")) {
    ...
    if (name !== "sage-conduct") {
      ...
      for (const sb of conductBlockList.length ? conductBlocks(body) : []) {
        if (sb.raw1.includes("sage-conduct") || sb.raw2.includes("sage-conduct")) continue;
  ...
  const commandedSkillNames = new Set<string>();
  ...
  if (existsSync(commandsRoot)) {
    for (const rec of skillRecords) {
      ...
      if (relDir.split("/")[0] === "catalog") continue; // catalog skills are retrieved by sage-recall, never commanded directly
      if (commandedSkillNames.has(rec.name) || SUPPORT_SKILLS.has(rec.name)) continue;
      issues.push({
        file: rec.skillMd,
        rule: "command-skill-pairing",
