...
export type GroundAction = "fix" | "annotate" | "confirm-intentional";

export interface GroundFlag {
  kind: "path" | "sha" | "scaffold" | "link";
  text: string;
  action: GroundAction;
  detail: string;
  line?: number;
...
function pathExists(root: string, cited: string, extraBase?: string): boolean {
  const bases = [root, extraBase].filter((b): b is string => !!b);
  if (cited.includes("*")) {
  ...
  for (const b of bases) {
    ...
    if (cited.endsWith(".html")) {
      ...
      try {
        if (existsSync(md) && statSync(md).isFile()) return true;
      } catch {
        /* continue */
      }
    }
...
export function groundMechanical(filePath: string, cwd?: string): GroundReport {
  ...
  for (const m of src.matchAll(PATH_CITE)) {
    const cited = m[1]!;
    if (cited.startsWith("http") || cited.includes("node_modules")) continue;
  ...
  for (const m of src.matchAll(HEX)) {
    const sha = m[1]!;
    if (/^[0-9]+$/.test(sha)) continue; // pure decimal, not a sha
...
export function formatGroundReport(report: GroundReport): string {
  if (!report.flags.length) return `sage ground ${report.path}\nno flags`;
  const rows = report.flags.map((f) => `- [${f.action}] ${f.kind} L${f.line ?? "?"}: ${f.detail}`);
  return `sage ground ${report.path}\n${report.flags.length} flag(s)\n${rows.join("\n")}`;
}

const DOC_DIMENSIONS = ["completeness", "consistency", "clarity", "scope", "feasibility"] as const;

export function reviewDocSkeleton(filePath: string, cwd?: string): string {
