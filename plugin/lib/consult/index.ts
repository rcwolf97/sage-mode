...
function recordConsultEgress(cwd: string, model: string, payload: { bytes: number; content_sha256: string; redactions: number }): void {
  try {
    recordEgress(cwd, {
      sink: "anthropic",
      model,
...
export function recordLaneCDispatch(
  root: string,
  opts: { model: string; sprint?: string; node?: string; payload: string },
): string {
