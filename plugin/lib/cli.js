...
function printFenced(verb, fenced, value, json, text) {
    if (fenced)
        process.stdout.write(`=== sage ${verb} begin ===\n`);
    print(value, json, text);
    if (fenced)
        process.stdout.write(`=== sage ${verb} end ===\n`);
}
...
function reportLedgerNotFound(verb, fenced, e, json) {
    if (json) {
...
function renderFilesReport(f) {
    const section = (label, paths) => paths.length ? `  ${label} (${paths.length}):\n${paths.map((p) => `    ${p}`).join("\n")}` : `  ${label} (0)`;
    return [
...
function renderHealthReport(h) {
    ...
    if (h.verify.length) {
        for (const v of h.verify)
            lines.push(`  ${v.key}: ${JSON.stringify(v.command)} resolvable=${v.resolvable}`);
    }
    else {
        lines.push("  none configured");
    }
    lines.push(`manifest: ${h.manifest.ownedClean} owned-clean, ${h.manifest.ownedModified} owned-modified, ${h.manifest.unowned} unowned, ${h.manifest.missing} missing`);
    lines.push(`gaps (${h.gaps.length}):`);
    lines.push(h.gaps.length ? h.gaps.map((g) => `  - ${g}`).join("\n") : "  none");
    lines.push(`ok: ${h.ok}`);
    return lines.join("\n");
}
function renderUninstallReport(r) {
    const lines = [];
...
async function main(argv) {
    ...
    if (cmd === "uninstall") {
        ...
        if (!yes && !json) {
            fail("sage uninstall removes files sage installed — re-run with --yes to confirm " +
    ...
    if (cmd === "unsafe") {
        ...
        writeFileSync(`${dir}/unsafe`, JSON.stringify({ reason, ts: new Date().toISOString(), once: true }) + "\n");
    ...
    if (cmd === "evidence") {
        ...
        if (sub === "trust") {
            ...
            if (!c)
                fail("sage evidence trust --command C");
            trust(c);
    ...
    if (cmd === "dag") {
        ...
        if (sub === "lanes") {
            if (!file)
                fail("sage dag lanes <dag.json> --wave N");
        ...
        if (sub === "worktree") {
            ...
            if (!nodeId || !dagFile)
                fail("sage dag worktree <nodeId> --dag <dag.json>");
    ...
    if (cmd === "review") {
        ...
        if (sub === "scope") {
            ...
            if (!base)
                fail("sage review scope --base <ref>");
            const fenced = flag(rest, "fenced");
        ...
        if (sub === "select") {
            ...
            if (!raw)
                fail("sage review select --scope <json>");
            const fenced = flag(rest, "fenced");
    ...
    if (cmd === "board") {
        ...
        if (sub === "status") {
            ...
            const state = opt(rest, "state");
            if (!sprint || !node || !state)
                fail("sage board status --sprint S --node N --state S");
        ...
        if (sub === "blocker") {
            const node = opt(rest, "node");
            if (!sprint || !node)
                fail("sage board blocker --sprint S --node N");
        ...
        if (sub === "answer") {
            const node = opt(rest, "node");
            if (!sprint || !node)
                fail("sage board answer --sprint S --node N");
            const body = await readStdin();
        ...
        if (sub === "ledger") {
            ...
            try {
                ...
                if (e instanceof LedgerNotFoundError)
                    return reportLedgerNotFound("board ledger", false, e, json);
                throw e;
            }
        }
        if (sub === "wtf") {
            ...
            try {
                ...
                printFenced("board wtf", fenced, result, json, `wtf=${result.score}% stopAndAsk=${result.stopAndAsk} hardCapReached=${result.hardCapReached}`);
                return 0;
            }
            catch (e) {
                if (e instanceof LedgerNotFoundError)
                    return reportLedgerNotFound("board wtf", fenced, e, json);
                throw e;
            }
        }
        if (sub === "render") {
    ...
    if (cmd === "egress") {
        ...
        if (sub === "list") {
            ...
            const text = rows.length
                ? renderTable(["seq", "ts", "sink", "model", "lane", "bytes", "redactions"], rows.map((r) => [String(r.seq), r.ts, r.sink, r.model, r.lane, String(r.bytes), String(r.redactions)]))
                : "no egress recorded";
        ...
        if (sub === "grants") {
            ...
            const text = g.length
                ? renderTable(["lane", "sink", "model", "revokeCommand"], g.map((x) => [x.lane, x.sink, x.model, x.revokeCommand]))
    ...
    if (cmd === "recall") {
        ...
        if (sub === "dedup") {
            ...
            if (!text)
                fail("sage recall dedup --applies-when TEXT");
