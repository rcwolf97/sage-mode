...
import { classify, installFile, plannedRemovals, readManifest, writeManifest, } from "../manifest/index.js";
...
function candidateFiles(sageHome, root, opts) {
    const out = [];
    ...
    if (existsSync(assetsSrc)) {
        for (const name of ["notebook.css", "mermaid.min.js"]) {
            ...
            if (existsSync(from))
                out.push({ relPath: join("docs", "assets", name), sourceAbsPath: from });
        }
    }
...
function recordResult(report, res) {
    if (res.action === "written")
        report.written.push(res.path);
    else if (res.action === "refreshed")
        report.refreshed.push(res.path);
    else
        report.preserved.push(res.path);
}
...
// collision.
function homeProjectCollision(root) {
    const userDir = resolve(sageUserDir());
    const projectSageDir = resolve(join(root, ".sage"));
    if (userDir !== projectSageDir)
        return null;
    return (`the user-level sage config directory (${userDir}) and this project's .sage directory ` +
...
export function setup(opts) {
    ...
    const report = { written: [], preserved: [], refreshed: [] };
...
function note(gaps, problems, message, severity) {
    gaps.push(message);
    if (severity === "broken")
...
export function checkHealth(opts) {
    ...
    const collision = homeProjectCollision(root);
    if (collision)
        note(gaps, problems, collision, "broken");
    const notebookRoot = projectDocsDir(root);
    let notebookSource = "default";
    ...
    if (!existsSync(projectCfgPath)) {
        ...
        try {
            projectCfg = readJson(projectCfgPath);
            if (projectCfg?.notebook?.root)
                notebookSource = "project-config";
        }
        catch {
    ...
    for (const i of interpreters) {
        if (!i.present) {
            ...
            // intentional absence.
            note(gaps, problems, `${i.name} is on PATH but failed to execute — broken install or wrong architecture`, "broken");
        }
    }
    ...
    if (!sageIgnored || !worktreesIgnored) {
        ...
        note(gaps, problems, ".sage/ and/or .worktrees/ are not gitignored — evidence fingerprints will silently grade STALE forever until this is fixed", manifestInstalled ? "broken" : "gap");
    }
    const verify = [];
    ...
    const tally = (cls) => {
        if (cls === "owned-clean")
            counts.ownedClean++;
        else if (cls === "owned-modified")
            counts.ownedModified++;
        else if (cls === "unowned")
            counts.unowned++;
        else
            counts.missing++;
    ...
    if (counts.ownedModified > 0) {
        note(gaps, problems, `${counts.ownedModified} project file(s) were customized after install and are preserved as-is`, "gap");
    }
    return {
        sageHome,
        project: root,
