...
6. **Write `<project>/.sage/config.json`** — the project profile, verify commands, and lane config that the rest of sage-mode reads. `sage setup` seeds `verify.tests`/`verify.typecheck`/`verify.build` with generic npm defaults, because those have a safe generic guess for most projects. It does **not** invent defaults for a non-`web` profile's own checks (`api`: `verify.contract`/`verify.migrations`/`verify.errors`; `cli`: `verify.golden`/`verify.sandbox`/`verify.ttfs`; `ai-product`: `verify.evals`/`verify.prompts`) — there is no generic command for "run this project's migration-safety analysis." If the project already has that tooling, ask for the real command and add it under `verify` in this file; if it doesn't, leave the key absent. `/sage-verify` treats an absent key as "not verified," never as a reason to skip the check silently — see `skills/sage-verify/references/profiles.md`.
