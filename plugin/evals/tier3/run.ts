...
import { renderLedger, ledgerPath, boardDir, loadLedger, writeBlocker, next as boardNext, type Ledger } from "../../lib/board/index.js";
...
function record(id: string, title: string, status: Status, detail: string): void {
...
function runCli(args: string[], opts: { cwd?: string; input?: string } = {}): { status: number; stdout: string; stderr: string } {
...
function runHook(hook: string, payload: unknown, cwd: string): { status: number; stdout: string; stderr: string } {
...
function scenario1(): void {
  ...
  try {
    ...
    for (let loop = 0; loop < 3; loop++) {
      ...
      const parsed = JSON.parse(out.stdout || "{}") as { followup_message?: string };
      assertTrue(
        typeof parsed.followup_message === "string" && parsed.followup_message.length > 0,
        `loop ${loop}: expected a followup_message forcing another turn (turn must not claim success), got: ${out.stdout}`,
      );
    }
    ...
    assertTrue(
      ledgerText.includes("WARNING — allowing after 3 blocked re-entries"),
      `expected the RED warning appended to the ledger after 3 loops, got:\n${ledgerText}`,
    );
...
function scenario4(): void {
  ...
  try {
    ...
    const decision = JSON.parse(hookOut.stdout || "{}") as { permission?: string; agent_message?: string };
    ...
    record(
      ...
      title,
      "PASS",
      "hooks/sage-lane denied the out-of-lane write (permission: deny, agent_message names the violation); the file never landed on disk; board.writeBlocker() produced n1.blocker.md recording the block.",
    );
  } catch (e) {
    record(id, title, "FAIL", (e as Error).message);
  } finally {
...
function scenario5(): void {
  ...
  try {
    ...
    const parsed = JSON.parse(out.stdout) as { violations: { code: string }[] };
    ...
    record(
      ...
      title,
      "PASS",
      `sage dag validate evals/fixtures/overlap-dag.json exited 1 with ${d2.length} D2 violation(s) — the graph is refused rather than presented for dispatch.`,
    );
  } catch (e) {
    record(id, title, "FAIL", (e as Error).message);
  }
...
function scenario7(): void {
  ...
  try {
    ...
    const graded = JSON.parse(check.stdout) as { grade: string; reason: string };
...
function scenario8(): void {
  ...
  try {
    ...
    const cliAction = JSON.parse(cliOut.stdout) as { action: string; nodes: string[] };
    ...
    record(
      ...
      title,
      "PASS",
      "board.next() (both as a direct call and via `sage board next`) resumed the in-flight node n1 from the ledger on disk alone — no in-process or session state was involved, which is exactly what a /clear mid-sprint requires.",
    );
  } catch (e) {
    record(id, title, "FAIL", (e as Error).message);
  } finally {
