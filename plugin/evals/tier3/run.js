...
function scenario2() {
    ...
    try {
        ...
        assertTrue(Array.isArray(envelope.findings), `expected a { findings, rejected } envelope from \`review gate --json\`, got: ${out.stdout.slice(0, 200)}`);
        const findings = envelope.findings;
        const rejected = envelope.rejected ?? [];
        assertTrue(rejected.length === 0, `expected the fixture row to be well-formed, but the gate rejected ${rejected.length}: ${JSON.stringify(rejected)}`);
