...
import { lint, findDocsRootLiterals, type LintIssue } from "../lib/lint/index.js";

const root = resolve(join(import.meta.dirname, ".."));

function issuesFor(rule: string, issues: LintIssue[]): LintIssue[] {
  return issues.filter((i) => i.rule === rule);
}

function describe(issues: LintIssue[]): string {
...
function skillMd(name: string, extraFrontmatter = "", body = ""): string {
  return `---
name: ${name}
...
test("reference-integrity: does not fire on a references/ file that exists, or a resolvable <placeholder> path", () => {
  ...
  writeFileSync(
    join(dir, "SKILL.md"),
    skillMd("stub", "", "See `references/real.md`. Profile at `profiles/<profile>.json`, e.g. `profiles/web.json`."),
  );
...
test("self-containment: does not fire on the allowlisted skills/<other>/references/* dispatch form, a glob, or an own-directory reference", () => {
  ...
  writeFileSync(
    ...
    skillMd(
      "skill-a",
      "",
      "Pass the path `skills/skill-b/references/checklists/x.md` to the dispatch. Also see `references/own.md` and `skills/skill-a/references/own.md`. Index everything under `skills/**/SKILL.md`.",
    ),
  );
  writeFileSync(join(b, "SKILL.md"), skillMd("skill-b"));
...
test("conduct-parity: does not fire when a skill cites rules/sage-conduct.mdc instead of restating it, or merely shares a topic", () => {
  ...
  writeFileSync(
    ...
    skillMd(
      "stub",
      "",
      "Follow the anti-sycophancy rule from `rules/sage-conduct.mdc` (not restated here). Separately, be skeptical of user claims in general — a loosely related sentiment, not the rule's wording.",
    ),
  );
...
test("conduct-parity against the real corpus: no SKILL.md restates a rules/sage-conduct.mdc heading or a near-duplicate 2+ sentence normative block", () => {
  ...
  assert.deepEqual(
    issues,
    [],
    `rules/sage-conduct.mdc says "Do not paste it into skills" — these skills paste it anyway:\n${describe(issues)}`,
  );
});
...
test("frontmatter-validity: fires on a SKILL.md name/directory mismatch, a missing agent name, and a commands/*.md with no description", () => {
  ...
  assert.ok(
    issues.some((i) => i.file.endsWith("skills/real-dir-name/SKILL.md") && /does not match its directory/.test(i.message)),
    `expected a SKILL.md name/directory mismatch, got:\n${describe(issues)}`,
  );
  assert.ok(
    issues.some((i) => i.file.endsWith("agents/reviewer.md") && /missing `name`/.test(i.message)),
    `expected a missing-name issue for the agent card, got:\n${describe(issues)}`,
  );
  assert.ok(
    issues.some((i) => i.file.endsWith("commands/do-thing.md") && /missing or empty `description`/.test(i.message)),
    `expected a missing-description issue for the command, got:\n${describe(issues)}`,
  );
});
...
test("roster-checklist-coverage: fires on a specialist with no checklist file AND a checklist file nothing references", () => {
  ...
  assert.ok(
    issues.some((i) => /"ghost-specialist" is referenced/.test(i.message)),
    `expected a missing-checklist issue for ghost-specialist, got:\n${describe(issues)}`,
  );
  assert.ok(
    issues.some((i) => /"orphan\.md" exists but is not referenced/.test(i.message)),
    `expected an orphaned-checklist issue for orphan.md, got:\n${describe(issues)}`,
  );
});
...
test("command-skill-pairing: fires on a command invoking a nonexistent skill AND a non-catalog skill with no command", () => {
  ...
  assert.ok(
    issues.some((i) => i.file.endsWith("commands/ghost.md") && /nonexistent-skill/.test(i.message)),
    `expected a broken-target issue for commands/ghost.md, got:\n${describe(issues)}`,
  );
  assert.ok(
    issues.some((i) => i.file.endsWith("skills/uncommanded/SKILL.md")),
    `expected an uncommanded-skill issue for skills/uncommanded, got:\n${describe(issues)}`,
  );
});
...
function loadAllowlist(): { file: string; literal: string }[] {
  ...
  return raw
    ...
    .map((l) => {
      const i = l.indexOf(": ");
      if (i < 0) throw new Error(`malformed notebook-root-allowlist.txt line (expected "<file>: <literal>"): ${l}`);
...
test("notebook-root-literal against the real corpus: every hardcode is on the allowlist, and every allowlist line still matches a real hardcode", () => {
  ...
  assert.deepEqual(
    notAllowlisted,
    [],
    `NEW hardcoded docs/-rooted notebook path(s) not in test/notebook-root-allowlist.txt — either fix the prose to stop hardcoding the root, or (only if genuinely a new, deliberate literal) add a line:\n  ${notAllowlisted.join("\n  ")}`,
  );
  ...
  assert.deepEqual(
    stale,
    [],
    `test/notebook-root-allowlist.txt has line(s) that no longer match an actual hardcode — the fix landed, delete the stale line(s) so the list keeps shrinking:\n  ${stale.join("\n  ")}`,
  );
});
