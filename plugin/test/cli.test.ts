...
test("sage board wtf is reachable from the CLI and returns a computed score", () => {
  ...
  writeFileSync(
    join(dir, ".sage/sprints/00/ledger.md"),
    "# Sprint 00\n\n## Nodes\n- n1: done\n\n## Circuit\n- WTF-LIKELIHOOD: 0\n",
  );
...
test("sage review recommendation is reachable from the CLI and fails a review missing the section", () => {
  ...
  assert.ok(out.issues.some((s: string) => /missing/i.test(s)));
});
...
function twoWaveDagFile(dir: string): string {
  const dagFile = join(dir, "dag.json");
  writeFileSync(
    dagFile,
    JSON.stringify({
...
test("dag plan waves and dag lanes --wave <i> agree for every wave index, and an out-of-range index reports the wave violation", () => {
  ...
  const { waves } = JSON.parse(planR.stdout) as { waves: string[][] };
  ...
  for (let i = 0; i < waves.length; i++) {
    ...
    const out = JSON.parse(lanesR.stdout) as { violations: unknown[] };
  ...
  const oob = JSON.parse(oobR.stdout) as { violations: { code: string; message: string }[] };
...
test("sage evidence run refuses with a clear message when .sage/ is not gitignored, and --allow-unignored proceeds anyway", () => {
  ...
  const allowed = spawnSync(
    "node",
    [CLI, "evidence", "run", "--label", "t", "--allow-unignored", "--", "node", "-e", "process.exit(0)"],
