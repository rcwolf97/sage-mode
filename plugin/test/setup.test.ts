...
// cards.
function makeSageHome(agentNames: string[] = DEFAULT_AGENTS): string {
...
function isolatedUserHome(): string {
...
function agentsRelPaths(agentNames: string[] = DEFAULT_AGENTS): string[] {
...
test("an untouched owned card is refreshed when the source changes", () => {
  ...
  assert.equal(
    readFileSync(join(dir, ".cursor", "agents", "reviewer.md"), "utf8"),
    "a newer version of the shipped card\n",
  );
...
test("a file present with no manifest entry (pre-manifest install) is preserved, not clobbered", () => {
  ...
  assert.equal(
    readFileSync(join(dir, ".cursor", "agents", "architect.md"), "utf8"),
    "a file that predates the manifest\n",
  );
...
test("a corrupt install-manifest.json makes every tracked file look unowned and untouched, never crashes setup", () => {
  ...
  assert.equal(
    readFileSync(join(dir, ".cursor", "agents", "architect.md"), "utf8"),
    "original content for architect.md\n",
  );
});
...
test("an interrupted setup (throw partway through the install loop) leaves the manifest exactly as it was before the run", () => {
  ...
  assert.equal(
    manifestAfter,
    manifestBefore,
    "manifest on disk must be exactly the pre-run version, not partially updated",
  );

  const manifest = readManifest(dir);
  for (const rel of [...agentsRelPaths(), ...ASSET_REL_PATHS]) {
    assert.ok(
      manifest.entries.some((e) => e.path === rel),
      `expected ${rel} still tracked from the successful prior run`,
    );
  }
...
test("setting up into a project whose .cursor/agents path is blocked by a plain file throws instead of silently reporting success", () => {
  ...
  assert.ok(
    !existsSync(join(dir, ".sage", "install-manifest.json")),
    "no manifest should be written for a failed run",
  );
});
...
test("interpreter detection actually executes the binary: present-but-broken is reported present:true, runs:false", () => {
  ...
  let report: ReturnType<typeof checkHealth>;
  try {
...
test("--purge-user-config removes only this project from trustedRoots, never the rest of ~/.sage/config.json", () => {
  ...
  const cfgBefore = JSON.parse(readFileSync(cfgPath, "utf8")) as {
    trustedRoots: string[];
    sageHome: string;
    version: string;
    installedAt: string;
  ...
  const cfgAfter = JSON.parse(readFileSync(cfgPath, "utf8")) as {
    trustedRoots: string[];
    sageHome: string;
    version: string;
    installedAt: string;
...
test("setup refuses, before writing anything, when HOME resolves to the project root itself", () => {
  ...
  assert.match((threw as Error).message, /HOME/);
  assert.match((threw as Error).message, new RegExp(dir.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
