...
function runAsClaude(hook, payload, tmp) {
    const r = spawnSync(join(hooks, hook), {
        input: payload,
        encoding: "utf8",
        env: {
    ...
    return { json: json, status: r.status };
}
...
test("sage-careful emits Claude Code deny/ask shape and exits 2 on deny", () => {
    ...
    const viaToolInput = runAsClaude("sage-careful", JSON.stringify({ hook_event_name: "PreToolUse", tool_name: "Bash", tool_input: { command: "rm -rf /" } }), tmp);
    ...
    for (const matchers of Object.values(manifest.hooks)) {
        for (const matcher of matchers) {
            for (const entry of matcher.hooks) {
                const cmd = entry.command;
                assert.ok(cmd.startsWith("${CLAUDE_PLUGIN_ROOT}/"), `hooks-claude.json command "${cmd}" expected to start with \${CLAUDE_PLUGIN_ROOT}/`);
