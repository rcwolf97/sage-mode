...
test("sage CLI end-to-end smoke pass", async (t) => {
  ...
  function sage(args: string[], opts?: { input?: string }): { status: number; stdout: string; stderr: string } {
  ...
  await t.test("1. setup --check on a bare repo exits 0, writes NOTHING, and reports gaps", () => {
    ...
    assert.equal(
      report.ok,
      true,
      "a fresh, never-set-up repo must report ok:true — absence of setup is a gap, not a failure",
    );
  ...
  await t.test("5. an interface-order violation is detected when a consumer stops depending on its producer", () => {
    ...
    const violations = JSON.parse(r.stdout).violations as Violation[];
  ...
  await t.test("6. a dependsOn typo (camelCase) is rejected as an unknown field, not a silent false PASS", () => {
    const typo: Record<string, unknown> = structuredClone(dagObj);
    const nodes = typo.nodes as Array<Record<string, unknown>>;
    ...
    const violations = JSON.parse(r.stdout).violations as Violation[];
    assert.ok(
      violations.some((v) => /unknown/i.test(v.message)),
  ...
  await t.test("8. review scope output contains no .sage/ or .worktrees/ paths", () => {
    ...
    const polluted = out.files.filter((f: string) => f.startsWith(".sage") || f.startsWith(".worktrees"));
  ...
  await t.test("10. egress grants returns a grant for each configured metered lane, every one with a revokeCommand", () => {
    ...
    assert.equal(r.status, 0, r.stdout + r.stderr);
    const grants = JSON.parse(r.stdout).grants as Array<{ lane: string; sink: string; model: string; revokeCommand: string }>;
    ...
    for (const g of grants) {
      ...
      assert.ok(
        typeof g.revokeCommand === "string" && g.revokeCommand.length > 0,
        `grant for lane ${g.lane} is missing a non-empty revokeCommand`,
      );
    }
  });
  ...
  await t.test("11. uninstall --yes removes clean installed files, PRESERVES the edited card, and is idempotent", () => {
    ...
    assert.ok(
      !existsSync(join(projectDir, ".cursor", "agents", "reviewer.md")),
      "a clean, untouched card must be removed",
    );
