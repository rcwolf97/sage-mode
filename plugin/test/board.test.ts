...
const pending = { status: "pending" as const, attempts: 0, updated: "" };
const done = { status: "done" as const, attempts: 1, updated: "", verify: "PASS" };
const blocked = { status: "blocked" as const, attempts: 1, updated: "" };
const building = { status: "building" as const, attempts: 1, updated: "" };

test("board next returns the correct action for fixture ledger states", () => {
  const cases: [string, Ledger, string][] = [
  ...
  for (const [name, ledger, action] of cases) {
    ...
    const decision = (function decide(l: Ledger) {
      const blockedIds = Object.entries(l.nodes).filter(([, x]) => x.status === "blocked").map(([id]) => id);
      if (blockedIds.length) return "rule";
      for (const wave of l.waves) {
        const states = wave.map((id) => l.nodes[id]?.status || "pending");
        if (states.some((s) => s === "building" || s === "claimed" || s === "in-review")) return "dispatch";
        if (states.every((s) => s === "done" || s === "abandoned")) continue;
        if (states.some((s) => s === "pending")) return "dispatch";
      }
      if (Object.values(l.nodes).every((x) => x.status === "done" || x.status === "abandoned")) return "done";
      return "dispatch";
    })(parsed);
...
function clean(over: Partial<WtfSignals> = {}): WtfSignals {
  return {
    reverts: 0,
...
function ledgerFor(sprint: string): Ledger {
  return {
    sprint,
