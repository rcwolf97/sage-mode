  ...
  cat >"$repo/.version-bump.json" <<'JSON'
{
  "files": [
    ...
    { "path": ".claude-plugin/plugin.yaml", "field": "version" }
  ],
  "audit": { "exclude": [] }
}
JSON
...
JSON
  printf '%s\n' "$yaml_body" >"$repo/.claude-plugin/plugin.yaml"
}

happy_repo="$TEST_ROOT/happy"
make_fixture "$happy_repo" $'name: sage-mode\nversion: 1.2.3'
...
[[ "$(yq -r '.version' "$happy_repo/.claude-plugin/plugin.yaml")" == "2.3.4" ]] \
...
make_fixture "$invalid_repo" $'name: sage-mode\nversion: 123'
cp "$invalid_repo/package.json" "$TEST_ROOT/package.before"
cp "$invalid_repo/.claude-plugin/plugin.yaml" "$TEST_ROOT/plugin.before"
...
cmp -s "$TEST_ROOT/plugin.before" "$invalid_repo/.claude-plugin/plugin.yaml" \
