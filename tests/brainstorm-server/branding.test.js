/**
 * Tests for the visual companion's Sage Mode/Prime Radiant branding.
 */
...
const ASSET_URL = 'https://primeradiant.com/brand/sage-mode-visual-brainstorming-logo.png';
...
function createPackagedServerFixture(version) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'sage-mode', version }, null, 2)
  );
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Sage Mode v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Sage Mode v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Sage Mode v/.test(html),
    'visible logo should appear before the Sage Mode version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Sage Mode v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Sage Mode branding'
  );
}
...
async function main() {
  ...
  await test('packaged plugin reads version from .claude-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes('Sage Mode vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
  ...
  await test('SAGE_MODE_DISABLE_TELEMETRY=true omits remote image but keeps local branding', async () => {
    const port = 3453;
    const dir = '/tmp/brainstorm-branding-disabled';
    await withServer({ port, dir, env: { SAGE_MODE_DISABLE_TELEMETRY: 'true' } }, async () => {
      writeFragment(dir);
      await sleep(300);
      const html = await fetchHtml(port);
  ...
  await test('SAGE_MODE_DISABLE_TELEMETRY=yes also omits the remote image on the waiting screen', async () => {
    const port = 3454;
    const dir = '/tmp/brainstorm-branding-disabled-waiting';
    await withServer({ port, dir, env: { SAGE_MODE_DISABLE_TELEMETRY: 'yes' } }, async () => {
      const html = await fetchHtml(port);
      assertBrandedFallbackText(html);
      assert(!html.includes(ASSET_URL), 'disabled telemetry should omit the remote image');
    });
