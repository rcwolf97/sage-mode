#!/usr/bin/env bash
# Regression check: Sage Mode should not route new worktrees through the old
...
assert_not_contains "$USING_SKILL" "~/.config/sage-mode/worktrees" "using-git-worktrees does not mention old global path"
...
assert_not_contains "$FINISHING_SKILL" "~/.config/sage-mode/worktrees" "finishing-a-development-branch does not treat old global path as owned"
...
assert_not_contains "$ROTOTILL_SPEC" "~/.config/sage-mode/worktrees" "rototill spec does not preserve old global path policy"
assert_not_contains "$ROTOTILL_PLAN" "~/.config/sage-mode/worktrees" "rototill plan does not preserve old global path policy"
