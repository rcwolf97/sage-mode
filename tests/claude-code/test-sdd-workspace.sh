...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.sage-mode/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.sage-mode/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.sage-mode/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.sage-mode/sdd/.gitignore" && "$(cat "$repo/.sage-mode/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .sage-mode/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .sage-mode/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.sage-mode/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.sage-mode/sdd/plan-a/review-"*.diff)
    ...
    if [[ "$wt_dir" == "$wt_root/.sage-mode/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
